import { AuthService, SkillService, TodoService, RoutineService, CheckinService } from '../services/index.js';
import { userSchema, loginSchema, skillSchema, todoSchema, routineSchema, checkinSchema } from '../schemas/validation.js';
...
export class SkillController {
    async getAll(req, res) {
        try {
            ...
            const skills = await skillService.getAll(category, tag);
            res.json(skills);
        }
        catch (error) {
            if (error instanceof Error) {
    ...
    async getOne(req, res) {
        try {
            ...
            if (error instanceof Error) {
                ...
                res.status(404).json({ error: 'Unknown server error' });
            }
        }
    }
...
export class TodoController {
    async getAll(req, res) {
        try {
            ...
            const todos = await todoService.getAll(userId, status);
            res.json(todos);
        }
        catch (error) {
            if (error instanceof Error) {
...
export class CheckinController {
    async getAll(req, res) {
        try {
            ...
            const checkins = await checkinService.getAll(userId, from, to);
            res.json(checkins);
        }
        catch (error) {
            if (error instanceof Error) {
